...
export class TabellaDatiImpiantiComponent {
  ...
  searchByTableFilters(tableFilters: any) {
    ...
    this.gestioneImpiantiService
      .getData(tableFilters.tableYear, tableFilters.tableMonth)
      .subscribe((response) => {
  ...
  calcolaSommaReparti(): void {
    this.kpi_rows.forEach((kpi: any) => {
      ...
      this.valoriKpiSommaReparti.set(`LLF${kpi.id}`, 0);
      this.valoriKpiSommaReparti.set(`NCV Cieffe${kpi.id}`, 0);
      this.valoriKpiSommaReparti.set(`NCV Ipsen${kpi.id}`, 0);
      this.valoriKpiSommaReparti.set(`TV${kpi.id}`, 0);
      this.valoriKpiSommaReparti.set(`ALU${kpi.id}`, 0);
      ...
      if (kpi.calcolato) {
        ...
        if (kpi.valori.length > 0) {
          ...
          this.calcKpi(kpi, 'LLF');
          this.calcKpi(kpi, 'NCV Cieffe');
          this.calcKpi(kpi, 'NCV Ipsen');
          this.calcKpi(kpi, 'TV');
          this.calcKpi(kpi, 'ALU');
        }
      }
    });
  }
