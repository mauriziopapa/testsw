...
const EnergiaConsumataMetanoService = require('../../../services/kpi/EnergiaConsumataMetanoService');
...
module.exports = () => {
  router.get('/debug', [isAuthenticated], async (req, res) => {
    try {
      ...
      const result = await EnergiaConsumataMetanoService.getDebugData({ from, to, target, kpi_id });
  ...
  router.get('/', [isAuthenticated], async (req, res) => {
    try {
      ...
      const result = await EnergiaConsumataMetanoService.getKpiValues({ yearFrom, yearTo, target, kpi_id });
      return res.json(result);
    } catch (error) {
