...
const PuntualitaFornitoriTSService = require('../../../services/kpi/PuntualitaFornitoriTSService');
...
module.exports = () => {
  router.get('/debug', [isAuthenticated], async (req, res) => {
    try {
      ...
      const result = await PuntualitaFornitoriTSService.getDebugData({ from, to, fornitore, rischio, target, kpi_id });
      return res.json(result);
    } catch (error) {
      log.error(`Error getting debug data for kpi Puntualita Fornitori TS: ${error.message}`, error);
  ...
  router.get('/', [isAuthenticated], async (req, res) => {
    try {
      const { from, to, fornitore, rischio, target, kpi_id } = req.query;
      const result = await PuntualitaFornitoriTSService.getKpiValues({ from, to, fornitore, rischio, target, kpi_id });
      return res.json(result);
    } catch (error) {
      log.error(`Error getting kpi Puntualita Fornitori TS: ${error.message}`, error);
      return res.sendStatus(500);
    }
  });
