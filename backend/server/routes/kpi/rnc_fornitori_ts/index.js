...
const RNCFornitoriTSService = require('../../../services/kpi/RNCFornitoriTSService');
...
module.exports = () => {
  router.get('/debug', [isAuthenticated], async (req, res) => {
    try {
      ...
      const result = await RNCFornitoriTSService.getDebugData({ from, to, fornitore, rischio, target, kpi_id });
      return res.json(result);
    } catch (error) {
      log.error(`Error getting debug data for kpi RNC Fornitori TS: ${error.message}`, error);
  ...
  router.get('/', [isAuthenticated], async (req, res) => {
    try {
      const { from, to, fornitore, rischio, target, kpi_id } = req.query;
      const result = await RNCFornitoriTSService.getKpiValues({ from, to, fornitore, rischio, target, kpi_id });
      return res.json(result);
    } catch (error) {
      log.error(`Error getting kpi RNC Fornitori Team System: ${error.message}`, error);
      return res.sendStatus(500);
    }
  });
  ...
  router.get('/fornitori_con_ordini_ts', [isAuthenticated], async (req, res) => {
    try {
      ...
      log.error(`Error getting kpi Fornitori Team System: ${error.message}`, error);
      return res.sendStatus(500);
    }
  });
