...
const TabellaGestioneImpiantiFVFService = require('../../services/TabellaGestioneImpiantiFVFService');
...
module.exports = () => {
  router.get('/', [isAuthenticated], async (req, res) => {
    ...
    try {
      ...
      const impianti = await TabellaGestioneImpiantiFVFService.getData(anno, mese);
      return res.json(impianti);
    } catch (error) {
      log.error(`Error finding data for tabella gestione impianti fvf: ${error.message}`, error);
  ...
  router.post('/', [isAuthenticated], async (req, res) => {
    ...
    try {
      ...
      const promises = [];
      promises.push(TabellaGestioneImpiantiFVFService.saveData(impianti));
      promises.push(KpiProduzioneService.upsert(kpi_produzione));
      const result = await Promise.all(promises);
      return res.json(result);
    } catch (error) {
      log.error(`Error updating data for tabella gestione impianti fvf: ${error.message}`, error);
      return res.sendStatus(500);
    }
  });
