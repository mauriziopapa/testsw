...
async function buildDataYear(dal, al, fornitore) {
  ...
  const { ordini, ordiniInRitardo } = data[0];

  const valueGroup = [new ValueGroup.Builder().setLabel('% Puntualità').setValori(valori).build()];
...
async function buildData(dal, al, fornitore) {
  ...
  let [ordini] = await Promise.all(promises);

  let ordiniInRitardo = ordini.filter((ordine) => ordine.lt_wd > ordine.lt_prod);
  const percPuntualita = (1 - ordiniInRitardo.length / ordini.length) * 100;

  let value = new Value.Builder().setLabel(label).setData(percPuntualita.toFixed(2)).build();
  // forzatura daniel ticket 1250
  if (moment(dal).year() === 2022 && moment(al).year() === 2022) {
    value = new Value.Builder().setLabel(label).setData(99.05).build();
    ordini = 485;
    ordiniInRitardo = 5;
  }

  return { value, ordini, ordiniInRitardo };
}

async function getKpi(dal, al, fornitore = '') {
  const query = `
  SELECT po.id, r.lt AS lt_prod, 
  (
    SELECT COUNT(*)-1 
    FROM tempo 
  ...
  WHERE r.deleted = 0 AND po.deleted = 0  AND 
  po.\`Status\` != 'In Corso' 
  ...
  AND (po.Vendor_id = :fornitore OR :fornitore = '-Tutti-') AND r.lt > 0`;
