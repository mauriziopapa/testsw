/* eslint-disable camelcase */
const moment = require('moment');
const { buildDalAl } = require('../../lib/time');
const TargetService = require('./TargetService');
const FornitoriTSService = require('../FornitoriTSService');
const OrdiniFornitoriTSService = require('./OrdiniFornitoriTSService');
const OrdiniMateriePrimeTSService = require('./OrdiniMateriePrimeTSService');

const Value = require('../../models/response/Value');
const ValueGroup = require('../../models/response/ValueGroup');

const config = require('../../config')[process.env.NODE_ENV || 'local'];

const log = config.log();

const FORMAT_DATE = 'YYYY-MM-DD';

module.exports.getDebugData = async (filters) => {
  const { dal, al } = buildDalAl(filters.from, filters.to);
  const { fornitore, rischio } = filters;
  let rischi = rischio;
  if (rischio === '-Tutti-') {
    rischi = ['A', 'B', 'C'];
  }

  const fornitoriByRischio = await FornitoriTSService.getFornitoriByRischio(rischi);

  const { percPiu2Giorni, percMeno2Giorni, ordini, ordiniInRitardo } = await getPuntualitaKpi(
    dal,
    al,
    fornitoriByRischio,
    fornitore
  );

  return { percPiu2Giorni, percMeno2Giorni, ordini, ordiniInRitardo };
};

module.exports.getKpiValues = async (filters) => {
  const { dal, al } = buildDalAl(filters.from, filters.to);
  const { fornitore, rischio, kpi_id } = filters;
  const inputTarget = filters.target;
  let rischi = rischio;
  if (rischio === '-Tutti-') {
    rischi = ['A', 'B', 'C'];
  }

  const promises = [];
  promises.push(TargetService.getTarget(inputTarget, kpi_id));
  promises.push(FornitoriTSService.getFornitoriByRischio(rischi));
  const [target, fornitoriByRischio] = await Promise.all(promises);

  const resultDati = await buildDataYear(dal, al, fornitoriByRischio, fornitore, target);
  return resultDati;
};

async function buildDataYear(dal, al, fornitoriByRischio, fornitore, target) {
  const dalPrec = moment(dal).subtract(1, 'year').format(FORMAT_DATE);
  const alPrec = moment(al).subtract(1, 'year').format(FORMAT_DATE);
  const dalPrec2 = moment(dal).subtract(2, 'year').format(FORMAT_DATE);
  const alPrec2 = moment(al).subtract(2, 'year').format(FORMAT_DATE);

  const years = [];
  years.push({ dal: dalPrec2, al: alPrec2 }, { dal: dalPrec, al: alPrec }, { dal, al });

  const promises = years.map((year) => buildData(year.dal, year.al, fornitoriByRischio, fornitore));
  const data = await Promise.all(promises);

  const { ordini, ordiniInRitardo } = data[2];

  const valueGroup = data.map((d) =>
    new ValueGroup.Builder().setLabel(`% Puntualità ${d.label}`).setValori(d.value).setTarget(target).build()
  );

  return { valueGroup, ordini, ordiniInRitardo };
}

async function buildData(dal, al, fornitoriByRischio, fornitore) {
  const dalAnno = moment(dal).year();
  const alAnno = moment(al).year();

  let label = `${dalAnno}-${alAnno}`;
  if (dalAnno === alAnno) {
    label = dalAnno;
  }

  const { percPiu2Giorni, percMeno2Giorni, ordini, ordiniInRitardo } = await getPuntualitaKpi(
    dal,
    al,
    fornitoriByRischio,
    fornitore
  );

  const valuePiu2 = new Value.Builder()
    .setLabel('assoluta')
    .setData(Math.round(percPiu2Giorni * 100))
    .build();
  const valueMeno2 = new Value.Builder()
    .setLabel('con rit <= 2 gg')
    .setData(Math.round(percMeno2Giorni * 100))
    .build();

  const value = new Value.Builder().setLabel('').setData([valuePiu2, valueMeno2]).build();

  return { value, ordini: ordini.length, ordiniInRitardo: ordiniInRitardo.length, label };
}

const getPuntualitaKpi = async (dal, al, fornitori, fornitore) => {
  const promises = [];
  promises.push(OrdiniFornitoriTSService.getOrdiniEvasi(dal, al, fornitore));
  promises.push(OrdiniMateriePrimeTSService.getOrdiniMateriePrimeEvasi(dal, al, fornitore));
  const [ordini, ordiniMP] = await Promise.all(promises);

  let fornitoriByRischio = fornitori;
  if (fornitore !== '-Tutti-') {
    fornitoriByRischio = fornitori.filter((f) => f.CodFornitore === parseInt(fornitore));
  }

  const orders = ordini.concat(ordiniMP);
  const filteredOrders = orders.filter((el) => fornitoriByRischio.some((f) => f.CodFornitore === el.IdFornitore));

  // Filtro per NumeroOrdine univoco perchè la query restituisce più righe
  const ordiniEffettivi = OrdiniFornitoriTSService.getOrdiniEffettivi(filteredOrders);
  const ordiniInRitardo = OrdiniFornitoriTSService.calculateOrdiniInRitardo(ordiniEffettivi);
  const ordiniInRitardoPiu2Giorni = OrdiniFornitoriTSService.calculateOrdiniInRitardoPiuDi2Giorni(ordiniEffettivi);
  const ordiniInRitardoMeno2Giorni = OrdiniFornitoriTSService.calculateOrdiniInRitardoMenoO2Giorni(ordiniEffettivi);
  const totPuntuali = OrdiniFornitoriTSService.calculatePuntualita(ordiniInRitardo, ordiniEffettivi);
  const puntPiu2Giorni = OrdiniFornitoriTSService.calculatePuntualita(ordiniInRitardoPiu2Giorni, ordiniEffettivi);
  const puntMeno2Giorni = OrdiniFornitoriTSService.calculatePuntualita(ordiniInRitardoMeno2Giorni, ordiniEffettivi);
  // il 74 % degli ordini e' puntuale
  // es. l'80% degli ordini ha una puntualita con tolleranza minore di 2 giorni puntPiu2Giorni
  // es. il 20% degli ordini ha una puntualita con tolleranza maggiore di 2 giorni puntMeno2Giorni
  // calcolo quindi la percentuale di queste sul totale dei puntuali
  const percPiu2Giorni = puntPiu2Giorni * totPuntuali;
  const percMeno2Giorni = puntMeno2Giorni * totPuntuali;
  return { percPiu2Giorni, percMeno2Giorni, ordini: ordiniEffettivi, ordiniInRitardo };
};

const getPuntualita = async (dal, al, fornitore) => {
  const promises = [];
  promises.push(OrdiniFornitoriTSService.getOrdiniEvasi(dal, al, fornitore));
  promises.push(OrdiniMateriePrimeTSService.getOrdiniMateriePrimeEvasi(dal, al, fornitore));
  const [ordini, ordiniMP] = await Promise.all(promises);

  // Filtro per NumeroOrdine univoco perchè la query restituisce più righe
  const ordiniEffettivi = OrdiniFornitoriTSService.getOrdiniEffettivi(ordini.concat(ordiniMP));
  const ordiniInRitardo = OrdiniFornitoriTSService.calculateOrdiniInRitardo(ordiniEffettivi);
  const puntualita = OrdiniFornitoriTSService.calculatePuntualita(ordiniInRitardo, ordiniEffettivi);
  return { puntualita, ordini: ordiniEffettivi.length, ordiniInRitardo: ordiniInRitardo.length };
};

module.exports.getPuntualita = getPuntualita;
