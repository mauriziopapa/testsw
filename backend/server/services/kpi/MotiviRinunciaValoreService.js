...
async function getKpi(anno, tipo_offerta) {
  ...
  SELECT 
    ...
    SUM(IFNULL(importo, 0)) AS valore
  FROM offerte
  WHERE YEAR(data_doc) = :anno 
    AND (esito NOT LIKE '%ok%' OR esito IS NULL OR Quote_Stage = 'Stand by')
    AND (Quote_Stage != 'Inviata' OR Quote_Stage IS NULL)
