...
async function buildDataReparto(reparto, dal, al, target) {
  ...
  // prendo la capacità teorica
  promises.push(getKpiReparto(reparto, dal, al, KPI_31));
  ...
  // capacità teorica anno prec
  promises.push(getKpiReparto(reparto, dal_prec, al_prec, KPI_31));
  ...
  // capacita_teorica - spento_standby
  const num = data[1] - data[0];
  // guasti
  const den = data[2];
  const num_prec = data[4] - data[3];
  const den_prec = data[5];
...
module.exports.getKpiValues = async (filters) => {
  ...
  promises.push(TargetService.getTarget(inputTarget, 158));
  const [reparti, target] = await Promise.all(promises);
