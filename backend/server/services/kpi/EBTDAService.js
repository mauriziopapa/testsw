...
async function buildDataAnno(tempo) {
  ...
  promises.push(TargetService.getTarget(tempo.anno, 103));
...
async function buildData(anno) {
  ...
  const valori = kpiData.map((data) =>
    ...
    new Value.Builder().setLabel(data.nome_azienda).setData(data.ebtda).build()
  );
...
async function getKpi(anno) {
  ...
  const query_kpi = `
  SELECT
    ROUND(concorrenti.ebtda_perc * 100, 2) as ebtda,
