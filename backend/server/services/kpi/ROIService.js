...
async function buildDataAnno(tempo) {
  ...
  promises.push(TargetService.getTarget(tempo.anno, 104));
...
async function buildData(anno) {
  ...
  const valori = kpiData.map((data) =>
    ...
    new Value.Builder().setLabel(data.nome_azienda).setData(data.roi).build()
  );
...
async function getKpi(anno) {
  ...
  const query_kpi = `
  SELECT
    ROUND(concorrenti.roi_perc * 100, 2) as roi,
